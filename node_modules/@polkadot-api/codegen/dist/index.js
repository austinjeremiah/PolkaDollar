...
const generateDescriptors = (lookupFn, checksumToIdx, typesBuilder, checksumBuilder, key, paths, genesis) => {
  ...
  const mapInteractions = (descriptor) => utils.filterObject(
    utils.mapObject(descriptor, (v) => Object.keys(v).map((v2) => `'${v2}'`)),
    (v) => v.length > 0
  );
  const allInteractions = {
    ...
    apis: mapInteractions(utils.mapObject(runtimeCalls, (v) => v.methods))
  };
...
`;
  return { descriptorTypes, descriptorValues, exports: exports$1, commonTypeImports };
};
