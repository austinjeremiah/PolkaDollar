...
async function detectPackageManager() {
  ...
  const version2 = await getVersion(executable);
  return detected = {
    packageManager,
    executable,
    version: version2
        ...
        Object.fromEntries(
          await Promise.all(
            Object.entries(config[kind] ?? {}).map(async ([key, v]) => [
              ...
              toHex(
                Blake2128(
                  Binary.fromText(await fs2.readFile(v, "utf-8")).asBytes()
                )
              )
            ])
          )
...
async function tagGenerated(descriptorsDir, chains, contracts, whitelist) {
  const filePath = join3(descriptorsDir, "generated.json");
  await fs2.writeFile(
    filePath,
    JSON.stringify({
      cliVersion,
...
async function alreadyGenerated(descriptorsDir, chains, contracts, whitelist) {
  const generatedJs\u00F4n = join3(descriptorsDir, "generated.json");
  if (!existsSync2(generatedJs\u00F4n)) return false;
  try {
    const generated = JSON.parse(
      await fs2.readFile(generatedJs\u00F4n, {
        encoding: "utf-8"
      })
    );
...
async function getPackageProtocol() {
  const { packageManager, version: version2 } = await detectPackageManager();
  switch (packageManager) {
    case "yarn":
      const yarnMajorVersion = Number(version2.split(".").at(0));
...
async function compileCodegen(packageDir) {
  ...
  const program2 = tsc.createProgramFromConfig({
    basePath: srcDir,
    compilerOptions: {
      skipLibCheck: true,
      declaration: true,
...
async function replacePackageJson(descriptorsDir, version2) {
  await fs2.writeFile(
    join3(descriptorsDir, "package.json"),
    `{
  "version": "0.1.0-autogenerated.${version2}",
...
function getCli({
  ...
  program.name("polkadot-api").description("Polkadot API CLI").version(version2);
...
//# sourceMappingURL=chunk-UMZZTPR7.js.map